...
import { getFirestore, collection, doc, onSnapshot, setDoc, deleteDoc, query, getDocs } from 'firebase/firestore';
import { getAuth, signInAnonymously, onAuthStateChanged, signInWithCustomToken } from 'firebase/auth';

// --- Ícones (Lucide React) ---
// Usando SVGs diretamente para não depender de importações externas no ambiente
...
// As variáveis __firebase_config e __app_id serão injetadas pelo ambiente.
const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : {};
const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-weekly-planner';
...
export default function App() {
    ...
    useEffect(() => {
        ...
        // Listener para configurações (total de horas)
        const unsubscribeSettings = onSnapshot(settingsDocRef, (docSnap) => {
            if (docSnap.exists()) {
                setTotalWeeklyHours(docSnap.data().totalWeeklyHours || 40);
            } else {
                // Define um valor padrão se não existir
                setDoc(settingsDocRef, { totalWeeklyHours: 40 });
            }
        }, (error) => console.error("Erro ao buscar configurações:", error));
    ...
    const handleTotalHoursChange = useCallback(async (e) => {
        ...
        if (db && userId) {
            const settingsDocRef = doc(db, 'artifacts', appId, 'users', userId, 'settings', 'userSettings');
            await setDoc(settingsDocRef, { totalWeeklyHours: Number(newTotal) }, { merge: true });
        }
    }, [db, userId]);
    ...
    const handleSaveTask = async (taskData) => {
        ...
        const docRef = doc(db, 'artifacts', appId, 'users', userId, 'tasks', taskToSave.id);
...
function TaskForm({ task, onSave, onClose }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (!formData.name || !formData.duration) {
            alert('Por favor, preencha o nome e a duração da tarefa.');
            return;
        }
        onSave(formData);
    };
                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value={formData.name} onChange={handleChange} required className="w-full bg-gray-700 text-white border-gray-600 rounded-md p-2 focus:ring-2 focus:ring-blue-500" />
                            </div>
                            <div className="flex gap-4">
                                <div className="flex-grow">
                                    <label htmlFor="duration" className="block text-sm font-medium text-gray-300 mb-1">Duração (horas/semana)</label>
                                    <input type="number" name="duration" id="duration" value={formData.duration} onChange={handleChange} required min="0.1" step="0.1" className="w-full bg-gray-700 text-white border-gray-600 rounded-md p-2 focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label htmlFor="color" className="block text-sm font-medium text-gray-300 mb-1">Cor</label>
                                    <input type="color" name="color" id="color" value={formData.color} onChange={handleChange} className="w-16 h-10 p-1 bg-gray-700 border-gray-600 rounded-md cursor-pointer" />
                                </div>
                            </div>
                            <div>
                                <label htmlFor="description" className="block text-sm font-medium text-gray-300 mb-1">Observação (Opcional)</label>
                                <textarea name="description" id="description" value={formData.description} onChange={handleChange} rows="3" className="w-full bg-gray-700 text-white border-gray-600 rounded-md p-2 focus:ring-2 focus:ring-blue-500"></textarea>
                            </div>
                        </div>
                    </div>
